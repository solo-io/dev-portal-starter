...
The validated variables are `VITE_API_PAGE_RELOAD`, `VITE_DEFAULT_APP_AUTH`, and `VITE_SESSION_EXPIRED_BEHAVIOR` in the list that follows, plus [`VITE_APPLIED_OIDC_AUTH_CODE_CONFIG`](#environment-variables-if-using-an-oidcauthorizationcode-authconfig), which selects the authentication flow. The two boolean variables accept `"true"`, `"1"`, `"false"`, and `"0"`. `VITE_SESSION_EXPIRED_BEHAVIOR` accepts one of the values listed with it, and `VITE_DEFAULT_APP_AUTH` accepts a comma-separated list of them. All four ignore case and surrounding whitespace, and treat an unset or empty value as their default.
...
- `VITE_DEFAULT_APP_AUTH` - This controls which credential sections are shown on the App details page. It takes a comma-separated list of `"OAUTH"`, `"API_KEY"`, and `"CLIENT_CREDENTIALS"`, so any combination can be shown — for example `"API_KEY,CLIENT_CREDENTIALS"`. `"ALL"` names every section and is the default. A name that is not one of these, or a list that names nothing, is a configuration error. This only controls what the App details page offers; hiding a section does not disable the credentials themselves, and any that already exist keep working.
